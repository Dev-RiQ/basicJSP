...
public class BoardDAO {
	...
	public String printPageRemote(int curPage) {
		...
		String tag = "<a href='/BoardMVC2/boardListPage.do?page=%s'>%s</a>";
	...
	private String getTableRow(Board board, int idx) {
		...
		for(String data : getAnBoardData(board)) {
			if(dateIdx++ == 3)
				sb.append("<td><a onclick=\"location.href='boardUpdateInfo.do?no="+idx+"'\">"+data+"</a></td>");
			else
				sb.append("<td>"+data+"</td>");
			
		}
		sb.append("<td>"
				+ "<button onclick=\"location.href='boardDelete.do?no="+idx+"'\">"
						+ "삭제하기</button></td>");
		sb.append("</tr>");
		return sb.toString();
	}
